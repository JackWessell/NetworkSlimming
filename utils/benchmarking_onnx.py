    ...
    try:
        ...
        for w in weight.ravel():
            if isinstance(w, np.int8):
                ...
                #I never reached this statement during my testing, but if you do you can add additional catches
...
def measure_inference_time(model_path, device, batch = 8):
   if device == "cpu":
       ...
       ort_provider = ['CUDAExecutionProvider', 'CPUExecutionProvider'] 
   ...
   print(f"Inference took {(end-start)/10} on {device} with batch size of {batch}")
...
print(f"Now benchmarking the model saved at {args.path}")
get_size(model_onnx)
